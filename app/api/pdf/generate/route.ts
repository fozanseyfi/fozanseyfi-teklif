...
export async function POST(request: NextRequest) {
  ...
  // Per-user hide/lock kontrolu — gizli veya kilitli projede PDF yok.
...
<div class="page">
  ...
  <div class="section">
    ...
    <div class="two-col">
      <div class="signature-box">
        <div class="signature-title">Teklifi Veren — ${project.organization.name}</div>
        <div style="margin-top:8px;font-size:9px;color:#9CA3AF;">
          ${project.organization.address ?? ""}<br>
          ${project.organization.phone ?? ""} · ${project.organization.email ?? ""}
        </div>
  ...
  <div class="footer-bar"><span>${project.organization.name} · ${formatDate(today)}</span><span>Sayfa 6</span></div>
</div>
